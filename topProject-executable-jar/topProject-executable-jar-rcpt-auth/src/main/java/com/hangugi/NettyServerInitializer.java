...
//	private static final Logger logger = LoggerFactory.getLogger(NettyServerInitializer.class);
	...
	public void initChannel(SocketChannel socketChannel) throws Exception {
		...
		//channelPipeline.addLast(new LoggingHandler(LogLevel.valueOf(this.workerLogLevel.toUpperCase())));
		channelPipeline.addLast(new DelimiterBasedFrameDecoder(maxFrameLength, Delimiters.lineDelimiter()));
		...
		channelPipeline.addLast(new ReadTimeoutHandler(this.serverIoTimeout));
		channelPipeline.addLast(this.nettyServerHandler);
	}
}